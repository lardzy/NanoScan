package com.gttcgf.nanoscan.guidingSteps;

import android.os.Bundle;
import android.view.LayoutInflater;
import android.view.View;
import android.view.ViewGroup;

import androidx.annotation.NonNull;
import androidx.annotation.Nullable;
import androidx.fragment.app.Fragment;

import com.gttcgf.nanoscan.MovingSpotsView;
import com.gttcgf.nanoscan.R;

public class FirstStepAragment extends Fragment {
    private MovingSpotsView movingSpotsView;
    public FirstStepAragment(){}

    public static FirstStepAragment newInstance() {
        return new FirstStepAragment();
    }

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_first_step, container, false);
        movingSpotsView = view.findViewById(R.id.movingSpotsView);
        return view;
    }

    @Override
    public void onResume() {
        super.onResume();
        movingSpotsView.post(new Runnable() {
            @Override
            public void run() {
                movingSpotsView.moveSpots();
            }
        });
    }
}